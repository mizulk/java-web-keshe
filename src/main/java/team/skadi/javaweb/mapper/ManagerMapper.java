...
public interface ManagerMapper {
	...
	Manager findManagerByWorkNumberAndPassword(@Param("workNumber") Short workNumber, @Param("password") String password);

	@Select("SELECT id, work_number, `password`, `name` FROM managers WHERE work_number = #{workNumber}")
	Manager findManagerByWorkNumber(@Param("workNumber") Short workNumber);
}
